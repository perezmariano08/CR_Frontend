...
import { dataTemporadasColumns } from '../../../Data/Temporadas/DataTemporadas';
...
const Temporadas = () => {
    ...
    const id = "id_temporada"
    const plural = "temporadas"
    const singular = "temporada"
    const get = "get-temporadas"
    const create = "crear-temporada"
    const importar = "importar-temporadas"
    const eliminar = "delete-temporada"
    ...
    const eliminarAños = async () => {
        if (selectedRows.length > 0) {
            ...
            const deletePromises = selectedRows.map(row => 
                Axios.post(`${URL}/admin/${eliminar}`, { id: row.id_temporada })
            );
    ...
    const agregarDato = async () => {
        if (
            ...
            try {
                ...
                const datoExiste = datosExistentes.some(a => a.torneo === torneo);
                if (datoExiste) {
        ...
        <Content>
            ...
            <ActionsCrud>
                ...
                <ActionsCrudButtons>
                    ...
                    <Button bg="export" color="white" onClick={handleExport} disabled={temporadasList.length === 0}>
                        <LuDownload />
                        <p>Descargar</p>
                    </Button>
                </ActionsCrudButtons>
            ...
            <Table data={temporadasList} dataColumns={dataTemporadasColumns} arrayName={singular.charAt(0).toUpperCase() + singular.slice(1)} id_={id} />
            {
                isCreateModalOpen && <>
